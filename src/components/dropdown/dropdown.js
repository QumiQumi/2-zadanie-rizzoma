...
document.addEventListener("DOMContentLoaded", function () {
    ...
    while (i < drop.length) {
        ...
        while (switchCount < switchArr.length) {
            ...
            minus = switchElem.querySelectorAll(
                ".dropdown-list__switcher_circle"
            )[0];
            plus = switchElem.querySelectorAll(
                ".dropdown-list__switcher_circle"
            )[1];
            number = switchElem.querySelector(
                ".dropdown-list__switcher_number"
            );

            minus.addEventListener("click", function () {
                number = event.target
                    .closest(".dropdown-list__switcher")
                    .querySelector(".dropdown-list__switcher_number")
                    .querySelector("h3");
...
function checkSum(event) {
    ...
    numbers = event.target
        ...
        .querySelectorAll(".dropdown-list__switcher_number");
...
function clearBtnInactive(event) {
    dropdown = event.target
        .closest(".dropdown-list")
        .querySelector(".dropdown-list__toolbar_clear");
